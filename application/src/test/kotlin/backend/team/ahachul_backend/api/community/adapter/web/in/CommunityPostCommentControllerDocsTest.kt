...
class CommunityPostCommentControllerDocsTest : CommonDocsTestConfig() {
            ...
            listOf(
                GetCommentsDto.CommentList(
                    parentComment = GetCommentsDto.Comment(
                        ...
                        upperCommentId = null,
                        content = "상위 내용",
                        imageUrls = listOf("https://cdn.ahhachul.com/comment/sample-parent.gif"),
                    ...
                    childComments = listOf(
                        GetCommentsDto.Comment(
                            ...
                            upperCommentId = 1,
                            content = "하위 내용",
                            imageUrls = listOf("https://cdn.ahhachul.com/comment/sample-child.png"),
                            status = CommentType.CREATED,
                            createdAt = LocalDateTime.now(),
                            createdBy = "작성자 ID",
                            writer = "작성자 닉네임",
    ...
    fun createCommunityPostCommentTest() {
        ...
        val response = CreateCommentDto.Response(
            ...
            imageUrls = listOf("https://cdn.ahhachul.com/comment/new-comment.gif"),
        )
    ...
    @Test
    fun updateCommunityPostCommentTest() {
        // given
        val response = UpdateCommentDto.Response(
            id = 2,
            content = "수정된 커뮤니티 코멘트 내용"
        )
        ...
        val request = UpdateCommentDto.Request(
            content = "수정할 커뮤니티 코멘트 내용"
        )

        // when
        val result = mockMvc.perform(
            patch("/v1/community-posts/{postId}/comments/{commentId}", 1L, 2L)
                .header("Authorization", "Bearer <Access Token>")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))
                .accept(MediaType.APPLICATION_JSON)
        ...
        result.andExpect(status().isOk)
            .andDo(
                document(
                    "update-community-post-comment",
                    getDocsRequest(),
                    getDocsResponse(),
                    pathParameters(
                        parameterWithName("postId").description("코멘트가 속한 게시글 아이디"),
    ...
    @Test
    fun deleteCommunityPostCommentTest() {
        // given
        ...
        // when
        val result = mockMvc.perform(
            delete("/v1/community-posts/{postId}/comments/{commentId}", 1L, 2L)
                .header("Authorization", "Bearer <Access Token>")
                .accept(MediaType.APPLICATION_JSON)
        )
