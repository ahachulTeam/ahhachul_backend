    ...
    fun getTodayCommuteCoachTest() {
        ...
        val response = CommuteCoachDto.Response(
            ...
            walkingMeta = CommuteCoachDto.WalkingMeta(
                totalWalkingMinutes = 16,
                source = CommuteCoachDto.WalkingLeg(
                    stationId = 1L,
                    stationName = "안암",
                    walkingMinutes = 9,
                    walkingMinutesSource = CommuteCoachDto.WalkingMinutesSource.USER_PROFILE,
                    walkingMinutesUpdatedAt = "2026-02-27T01:10:00",
                ),
                destination = CommuteCoachDto.WalkingLeg(
                    stationId = 2L,
                    stationName = "성수",
                    walkingMinutes = 7,
                    walkingMinutesSource = CommuteCoachDto.WalkingMinutesSource.DEFAULT,
                ...
                document(
                    ...
                    responseFields(
                        *commonResponseFields(),
                        ...
                        fieldWithPath("result.walkingMeta").type(JsonFieldType.OBJECT).description("도보 메타 정보").optional(),
                        fieldWithPath("result.walkingMeta.totalWalkingMinutes").type(JsonFieldType.NUMBER).description("출발+도착 도보 합계(분)").optional(),
                        fieldWithPath("result.walkingMeta.source.stationId").type(JsonFieldType.NUMBER).description("출발 도보 기준 역 ID").optional(),
                        fieldWithPath("result.walkingMeta.source.stationName").type(JsonFieldType.STRING).description("출발 도보 기준 역 이름").optional(),
                        fieldWithPath("result.walkingMeta.source.walkingMinutes").type(JsonFieldType.NUMBER).description("출발 도보 시간(분)").optional(),
                        fieldWithPath("result.walkingMeta.source.walkingMinutesSource").type(JsonFieldType.STRING).description("출발 도보 소스(USER_PROFILE|DEFAULT)").optional(),
                        fieldWithPath("result.walkingMeta.source.walkingMinutesUpdatedAt").type(JsonFieldType.STRING).description("출발 도보 마지막 갱신 시각").optional(),
                        fieldWithPath("result.walkingMeta.destination.stationId").type(JsonFieldType.NUMBER).description("도착 도보 기준 역 ID").optional(),
                        fieldWithPath("result.walkingMeta.destination.stationName").type(JsonFieldType.STRING).description("도착 도보 기준 역 이름").optional(),
                        fieldWithPath("result.walkingMeta.destination.walkingMinutes").type(JsonFieldType.NUMBER).description("도착 도보 시간(분)").optional(),
                        fieldWithPath("result.walkingMeta.destination.walkingMinutesSource").type(JsonFieldType.STRING).description("도착 도보 소스(USER_PROFILE|DEFAULT)").optional(),
                        fieldWithPath("result.walkingMeta.destination.walkingMinutesUpdatedAt").type(JsonFieldType.STRING).description("도착 도보 마지막 갱신 시각").optional(),
