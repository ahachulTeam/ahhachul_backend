...
@RestController
class LostPostBookmarkController(
    private val articleBookmarkUseCase: ArticleBookmarkUseCase
) {

    @Authentication
    @PostMapping("/v1/lost-posts/{postId}/bookmark")
    fun bookmark(@PathVariable postId: Long): CommonResponse<*> {
        articleBookmarkUseCase.bookmark(ArticleType.LOST, postId)
        return CommonResponse.success()
    }

    @Authentication
    @DeleteMapping("/v1/lost-posts/{postId}/bookmark")
    fun unbookmark(@PathVariable postId: Long): CommonResponse<*> {
        articleBookmarkUseCase.unbookmark(ArticleType.LOST, postId)
        return CommonResponse.success()
    }
}
